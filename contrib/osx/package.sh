...
if [ -z "$1" ]; then
    echo "Usage: $0 Electrum_BDRcoin.app"
    exit -127
fi
...
if ! which ${genisoimage} > /dev/null 2>&1; then
	mkdir -p /tmp/electrum-bdrcoin-macos
	cd /tmp/electrum-bdrcoin-macos
...
rm -rf /tmp/electrum-bdrcoin-macos/image > /dev/null 2>&1
mkdir /tmp/electrum-bdrcoin-macos/image/
cp -r $1 /tmp/electrum-bdrcoin-macos/image/
...
${genisoimage} \
    ...
    -dir-mode 0755 \
    -apple \
    -o Electrum-bdrcoin_uncompressed.dmg \
    /tmp/electrum-bdrcoin-macos/image || fail "Unable to create uncompressed dmg"

dmg dmg Electrum-bdrcoin_uncompressed.dmg electrum-bdrcoin-$VERSION.dmg || fail "Unable to create compressed dmg"
rm Electrum-bdrcoin_uncompressed.dmg

echo "Done."
sha256sum electrum-bdrcoin-$VERSION.dmg
