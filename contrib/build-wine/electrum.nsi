...
;Variables
  ...
  !define PRODUCT_NAME "Electrum-BDRcoin"
  !define PRODUCT_WEB_SITE "https://github.com/bdrcoin/electrum-bdrcoin"
  !define PRODUCT_PUBLISHER "BDRcoin"
...
;General
  ...
  ;Name and file
  Name "${PRODUCT_NAME}"
  OutFile "dist/electrum-bdrcoin-setup.exe"
...
;Interface Settings
  ...
  !define MUI_ICON "c:\electrum-bdrcoin\electrum\gui\icons\electrum.ico"
...
Section
  ...
  File "c:\electrum-bdrcoin\electrum\gui\icons\electrum.ico"
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-bdrcoin-${PRODUCT_VERSION}.exe" ""

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-bdrcoin-${PRODUCT_VERSION}.exe" "" "$INSTDIR\electrum-bdrcoin-${PRODUCT_VERSION}.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME} Testnet.lnk" "$INSTDIR\electrum-bdrcoin-${PRODUCT_VERSION}.exe" "--testnet" "$INSTDIR\electrum-bdrcoin-${PRODUCT_VERSION}.exe" 0


  ;Links bitcoin: URI's to Electrum-bdrcoin
  WriteRegStr HKCU "Software\Classes\bdrcoin" "" "URL:bdrcoin Protocol"
  WriteRegStr HKCU "Software\Classes\bdrcoin" "URL Protocol" ""
  WriteRegStr HKCU "Software\Classes\bdrcoin" "DefaultIcon" "$\"$INSTDIR\electrum.ico, 0$\""
  WriteRegStr HKCU "Software\Classes\bdrcoin\shell\open\command" "" "$\"$INSTDIR\electrum-bdrcoin-${PRODUCT_VERSION}.exe$\" $\"%1$\""
