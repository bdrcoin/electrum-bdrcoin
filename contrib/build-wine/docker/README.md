...
2. Build image
    ...
    ```
    $ sudo docker build -t electrum-bdrcoin-wine-builder-img contrib/build-wine/docker
    ```
...
3. Build Windows binaries
    ...
    $ FRESH_CLONE=contrib/build-wine/fresh_clone && \
        ...
        mkdir -p $FRESH_CLONE && \
        cd $FRESH_CLONE  && \
        git clone https://github.com/bdrcoin/electrum-bdrcoin.git && \
        cd electrum-bdrcoin
    ```
    ...
    $ git checkout $REV
    $ sudo docker run -it \
        --name electrum-bdrcoin-wine-builder-cont \
        -v $PWD:/opt/wine64/drive_c/electrum-bdrcoin \
        --rm \
        --workdir /opt/wine64/drive_c/electrum-bdrcoin/contrib/build-wine \
        electrum-bdrcoin-wine-builder-img \
        ./build.sh
    ```
...
Electrum-BDRcoin Windows builds are signed with a Microsoft Authenticode™ code signing
certificate in addition to the GPG-based signatures.

The advantage of using Authenticode is that Electrum-BDRcoin users won't receive a 
...
|-----------------------------------------------------------------------------------------------|
| Upload signatures to repo, as `$version/$filename.$builder.asc`         |
